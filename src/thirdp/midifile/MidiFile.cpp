...
namespace smf {
	...
	void MidiFile::readFromArray(std::vector<char>& vec) {
		std::stringstream in;
		in.write(&vec[0], vec.size());
		m_events.resize(m_trackCount);
		for (int i = 0; i < m_trackCount; i++) {
			m_events[i] = new MidiEventList;
		}
	...
	bool MidiFile::write(const std::string& filename) {
		...
		MLOG("Trying to write preset to " + filename);
		if (!output.is_open()) {
			std::cerr << "Error: could not write: " << filename << std::endl;
			MLOG("Error writing preset to " + filename);
			return false;
		}
		m_rwstatus = write(output);
		output.close();
		MLOG("Successfully wrote " + filename);
		return m_rwstatus;
	}
	...
	void MidiFile::writeToArray(std::vector<char>& vec) {
		std::stringstream out;
		write(out);
		auto written = out.tellp();
		vec.resize(written);
		out.read(&vec[0], written);
	}
	...
	bool MidiFile::write(std::ostream& out) {
		...
		for (i = 0; i < getNumTracks(); i++) {
			trackdata.reserve(123456);   // make the track data larger than
										 // expected data input
			trackdata.clear();
			MLOG("Events size: " + std::to_string((int)m_events[i]->size()));
